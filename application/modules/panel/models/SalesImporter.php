		...
		foreach ($importedData as $row) {
			...
			if ($exists instanceof Zend_Db_Table_Rowset_Abstract) {
				if ($exists->count() == 0) {
					...
					$row[1], $row[2], hexdec(sha1($row[1] . ' ' . $row[2])));
					if ($attendeeId == - 1) {
						// failed!
						$log .= 'Attendee ' . $row[1] .
				...
				$activation = $this->Tickets->activate($event, (int) $row[0], $attendeeId, $user);
				if($activation != $this->Tickets->getStatusCode('active')) {
 					$log .= 'Ticket ' . $row[0] . " was not activated successfully. Code $activation\n";
				}
			} else {
