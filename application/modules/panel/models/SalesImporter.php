		...
		foreach ($importedData as $row) {
			...
			if ($exists instanceof Zend_Db_Table_Rowset_Abstract) {
				if ($exists->count() == 0) {
					// not found; make one
					/*
					 * $attendeeId = $this->Attendees->create( $row[1], $row[2],
					 * 'bts-gen-' . sha1($row[1] . ' ' . $row[2])); if
					 * ($attendeeId == - 1) { // failed! $log .= 'Attendee ' .
					 * $row[1] . ' ' . $row[2] . " could not be created.\n";
					 */
				...
				$activation = $this->Tickets->activate($event, (int) $row[0], 
						$attendeeId, $user);
