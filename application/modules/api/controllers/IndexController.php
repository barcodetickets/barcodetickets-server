...
class Api_IndexController extends Api_Controller_Abstract
{
	public function testAction ()
	{
		...
		$this->view
			->assign('timestamp', $this->clientAuth
			->validateTimestamp($this->_getParam('timestamp')));
		try {
			$message = $this->clientAuth
				->generateSignature($_SERVER['REQUEST_METHOD'], $_SERVER['SERVER_NAME'], strtok($_SERVER['REQUEST_URI'], '?'), array(
		...
		$this->view
			->assign('signature_match', $this->clientAuth
			->validateSignature($_SERVER['REQUEST_METHOD'], $_SERVER['SERVER_NAME'], strtok($_SERVER['REQUEST_URI'], '?'), array(
			'sysName' => $this->_getParam('sysName') ,
			'signature' => $this->_getParam('signature') ,
			'timestamp' => $this->_getParam('timestamp'))));
	}
