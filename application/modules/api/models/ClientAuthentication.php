...
class Api_Model_ClientAuthentication
{
	...
	public function destroySession ($sessionId, $sysName)
	{
		$db = $this->Clients
			->getDb();
		$delete = $db->query('DELETE bts_sessions FROM bts_sessions NATURAL JOIN bts_clients'
		. ' WHERE bts_sessions.session_id = UNHEX(?) AND bts_clients.sys_name = ?', array(
			$sessionId ,
			$sysName));
	...
	public function validateSession ($sessionId, $sysName)
	{
		$db = $this->Clients
			->getDb();
		$select = $db->select()->from('bts_sessions', 'COUNT(*)')
		->where('expire_time > UTC_TIMESTAMP()')
		->where('session_id = UNHEX(?)', $sessionId)
		->joinNatural('bts_clients', array('sys_name'))
