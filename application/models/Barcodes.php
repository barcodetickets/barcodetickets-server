...
class Bts_Model_Barcodes
{
	...
	/**
	 * The cipher used across the installation for encryption of barcodes.
	 * While this is configurable, <em>end users should NOT reconfigure this
	 * setting because it WILL break existing barcodes and may break on
	 * upgrades</em>. It is thus explicitly not an option in the .ini config
	 * files.
	 *
	 * @var string
	 */
	...
	/**
	 * The block encryption mode used for encryption of barcodes.
	 * Again, end users should NOT modify this setting.
	 *
	 * @var string
	 */
	 ...
	 *        	(optional)
	 * @return string false as a binary string or false if not found
	 */
	protected function _retrieveEventHash ($eventId, $binary = true)
	{
		...
		if (array_key_exists($eventId, $this->_eventHashCache)) {
			...
			$hash = $this->EventsTable->select(false)
	 ...
	 * @return array false with the event, batch and ticket, or false on failure
	 */
	public function decryptBarcode ($barcodeString)
	{
		...
		/*
		 * at this point $parts will contain: [0] => full $barcodeString [1] =>
		 * eventId [2] => batchId [3] => encrypted portion
		 */
		$eventHash = $this->_retrieveEventHash($parts[1]);
		// if an eventHash cannot be found, we can't proceed with decryption
		if ($eventHash === false || empty($eventHash))
			return false;
			// DECRYPT
		$decryptedString = trim(
				mcrypt_decrypt(self::CIPHER, $eventHash, 
						base64_decode($parts[3]), self::MODE));
		$result = array();
		...
		if ($result[1] != $parts[1] || $result[2] != $parts[2]) {
			// results of decryption produce different data from the plaintext
			// wrapper text
			// = failure
			return false;
		}
		/*
		 * $result currently contains [0] => $decryptedString [1] => eventId [2]
		 * => batchId [3] => ticketId
		 */
		return array(
				'event' => $result[1],
	 ...
	 * @return array boolean containing event, batch, ticket and checksum, or
	 *         false on failure
	 */
			...
			$eventHash = null)
	{
		...
		// create our "checksum" from the first and last chars of the
	 ...
	 * generated on the spot with the same algorithm.
	 * Does not use the checksum column on the tickets table.
	 *
	 * @param int $eventId        	
	 * @param int $batchId        	
	 * @param string $ticketId        	
	...
	public function verifyChecksum ($eventId, $batchId, $ticketId, $checksum)
	{
		if (empty($eventId) || empty($batchId) || empty($ticketId) ||
			...
			throw new Bts_Exception(
					...
					Bts_Exception::BARCODES_PARAMS_BAD);
			/*
		 * $eventId = (int) $eventId; $batchId = (int) $batchId; $eventHash =
		 * $this->_retrieveEventHash($eventId); if ($eventHash === false) throw
		 * new Bts_Exception( 'Invalid eventId in
		 * Bts_Model_Barcodes::verifyChecksum()',
		 * Bts_Exception::BARCODES_EVENT_BAD); $baseString = $eventId . '-' .
		 * $batchId . '-' . $ticketId; $encryptedString =
		 * bin2hex(mcrypt_encrypt(self::CIPHER, $eventHash, $baseString,
		 * self::MODE));
		 */
