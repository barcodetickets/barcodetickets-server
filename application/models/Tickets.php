...
class Bts_Model_Tickets
{
	...
	protected $statuses = array(
			...
			255 => 'invalidother'
	); // 255 = unknown status but nevertheless invalid --
	                        // must be last
	...
	public function activate ($event, $ticket, $attendee, $user)
	{
		...
		if (! is_null($row)) {
			...
			return $this->getStatusCode('active'); // only
			                                       // $this->getStatusCode('active')
			                                       // means success
		} else
	...
	public function insert ($batch, $event, $status = 0)
	{
		...
		$checksum = $this->Barcodes->generateChecksum($event, $batch, $ticketId);
	...
	public function getByAttendee ($attendee)
	{
		...
		$select = $this->TicketsTable->select(true)->where('attendee_id = ?', 
				$attendee);
